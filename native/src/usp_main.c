...
int set_optr_via_ioctl(const char *optr_value) {
    ...
    if (fd >= 0) {
        ...
        if (name == NULL) {
            ALOGE("[USP] memory allocation failed: name");
            goto err_fd;
        }
        value = (char*) malloc(BUF_MAX_LEN);
        if (value == NULL) {
            ALOGE("[USP] memory allocation failed: value in fd>=0");
            goto err_fd;
        }
        ...
        if (strlen(NAME_E)+1 > BUF_MAX_LEN) {
            ALOGE("[USP] memcpy size failed:%d", strlen(NAME_E)+1);
            goto err_fd;
        }
        memcpy(name,NAME_E,strlen(NAME_E)+1);
        if (strlen(optr_value)+1 > BUF_MAX_LEN) {
            ALOGE("[USP] memcpy size failed:%d", strlen(optr_value)+1);
            goto err_fd;
        }
        memcpy(value,optr_value,strlen(optr_value)+1);
        value[strlen(optr_value)] = '\0';
    ...
    if (fd2 >= 0) {
        value = (char*) malloc(BUF_MAX_LEN);
        if (value == NULL) {
            ALOGE("[USP] memory allocation failed: value in fd2>=0");
            goto err_fd2;
        }
        memset(value,0x00,BUF_MAX_LEN);
        if (strlen(optr_value)+1 > BUF_MAX_LEN) {
            ALOGE("[USP] (fd2) memcpy size failed:%d", strlen(optr_value)+1);
            goto err_fd2;
        }
        memcpy(value,optr_value,strlen(optr_value)+1);
        value[strlen(optr_value)] = '\0';
        ...
        if (size<0 || size>8) {
            ALOGE("[USP] snprintf write failed, size:%d", size);
            goto err_fd2;
        }
...
int main(int argc, char **argv)
{
    ...
    if (strlen(propStr) > 0) {
        ...
        int n = sprintf(propSetStr, "%d" , propValue);
        if (n < 0) {
            ALOGE("[USP] sprintf failed:%d", n);
            return 0;
        }
